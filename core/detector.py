...
MODEL_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "face_landmarker.task"
)
...
class FaceDetector:
    ...
    def __init__(self, max_faces=1, min_detection_confidence=0.5,
        ...
        options = mp_vision.FaceLandmarkerOptions(
            ...
            output_facial_transformation_matrixes=True,  # nécessaire pour la pose de la tête
        )
    ...
    def get_transformation_matrix(self, result):
        """
        Retourne la matrice de transformation faciale 4x4 (numpy array) du
        premier visage détecté, ou None si indisponible.

        Cette matrice encode la rotation/translation 3D du visage par
        rapport à la caméra - elle sert à calculer pitch/yaw/roll (head pose).
        """
        if not result.facial_transformation_matrixes:
            return None
        return np.array(result.facial_transformation_matrixes[0])
