...
Le score combine plusieurs signaux :
  - PERCLOS (% de temps yeux fermés sur 1 min) -> métrique de référence NHTSA
  - Fermeture ponctuelle prolongée des yeux (signal immédiat -> micro-sommeil)
  - Posture de la tête (tête qui tombe/penche -> signal précoce)
...
WEIGHT_PERCLOS = 30        # métrique de référence scientifique (NHTSA)
WEIGHT_EYES_CLOSED = 20    # fermeture ponctuelle prolongée (signal immédiat)
WEIGHT_HEAD_POSE = 20      # tête qui tombe / penche (signal précoce)
WEIGHT_BLINK_RATE = 10     # clignements anormalement fréquents
WEIGHT_YAWN_RATE = 20      # bâillements répétés
...
class FatigueScorer:
    ...
    def compute(self, eye_state, yawn_state, head_state=None,
                perclos_state=None, current_time=None):
        """
        Calcule le score de fatigue à partir des états retournés par
        EyeStateTracker.update(), YawnTracker.update(), et (optionnellement)
        HeadPoseTracker.update() et PerclosTracker.update().
        ...
        :param perclos_state: dict retourné par PerclosTracker.update(), ou None
                               si le PERCLOS n'est pas suivi.
        ...
        # --- Facteur 1 : PERCLOS (métrique de référence NHTSA) ---
        # Score proportionnel au PERCLOS lui-même, plafonné à 1.0 dès que
        # le seuil d'alerte (20% par défaut) est atteint ou dépassé.
        if perclos_state is None:
            perclos_factor = 0.0
        else:
            perclos = perclos_state["perclos"]
            alert_threshold = 0.20  # doit rester cohérent avec PERCLOS_ALERT_THRESHOLD
            perclos_factor = min(perclos / alert_threshold, 1.0) if alert_threshold > 0 else 0.0

        # --- Facteur 2 : yeux fermés prolongés (alerte ponctuelle immédiate) ---
        ...
        # --- Facteur 3 : posture de la tête (pitch/yaw/roll anormaux) ---
        ...
        # --- Facteur 4 : taux de clignement récent ---
        ...
        # --- Facteur 5 : taux de bâillement récent ---
        ...
        return {
            ...
            "level": level,
            "details": {
                "perclos_factor": round(perclos_factor, 2),
